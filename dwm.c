...
#define XS_VOmute XF86XK_AudioMute
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			else /* center window title + 20 pixels seems to center better idk why*/
				drw_text(drw, x, 0, w, bh, (20 + w - TEXTW(m->sel->name)) / 2, m->sel->name, 0);
			if (m->sel->isfloating)
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? m->ww * m->mfact : 0; /*-3 removes gap between master and slaves*/
	else
		mw = m->ww;
